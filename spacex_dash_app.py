...
def get_scatter_plot(entered_site,payload):
    ...
    if entered_site == 'All Sites':
        fig3 = px.scatter(filtered_df, x='Payload Mass (kg)', 
        y='class', 
        color="Booster Version Category",
        title='Relationship between Payload Mass and Sucess Rate')
        return fig3
    else:  
        filtered_df= spacex_df.loc[spacex_df["Launch Site"]==entered_site] 
        filtered_df = filtered_df[(filtered_df["Payload Mass (kg)"] >= payload[0]) & (filtered_df["Payload Mass (kg)"] <= payload[1])]
        fig4 = px.scatter(filtered_df, x='Payload Mass (kg)', 
        y='class', 
        color="Booster Version Category",
        title='Relationship between Payload Mass and Sucess Rate in {}'.format(entered_site))
        return fig4
